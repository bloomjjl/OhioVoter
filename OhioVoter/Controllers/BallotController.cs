...
namespace OhioVoter.Controllers
{
    public class BallotController : Controller
    {
        ...
        {
            string emailAddress = ballotVM.VoterEmailAddress;
            string emailSubject = string.Format("Sample ballot for {0} Election", ballotVM.VotingDate);
            string emailBody = string.Format("{0}", GetEmailBodyForBallot(ballotVM));

            Services.Email email = new Services.Email();
            return email.SendEmail(emailAddress, emailSubject, emailBody);
        }
